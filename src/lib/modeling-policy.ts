...
export const browserModelingInstructions = `When browserModeling is true, use kind generated with a browser-manifold recipe shaped as {backend:"browser-manifold",recipe:{version:1,revision,output,nodes}} for genuinely new shapes. The browser builder supports only centered box(size), sphere(radius, segments), cylinder(radius, depth, axis x/y/z), extrude(profile, depth), transform(input, position, rotation, scale), and boolean union/subtract/intersect nodes. Keep recipes within 64 nodes, depth 16, segments 64, and the evaluator's 100,000-triangle and 8 MiB intermediate mesh limits; the baked GLB must stay within 2 MiB. Extrude a simple non-self-intersecting XY polygon with 3–64 distinct points along Z, centered from -depth/2 to +depth/2; do not repeat the first point, and do not request holes or twist. Dimensions and positions are meters in a Y-up world; rotations are radians using XYZ order; scale is positive. Reserve the entity first without geometry, then send one refined browser job; do not send a coarse browser job. Never supply model metadata: the trusted browser builder creates it. Every node ID must be unique. recipe.output must exactly match an existing node ID; every node must be reachable from that output. Node IDs are separate from the scene entity ID. For one-node extrusion, use output:"mesh" and nodes:[{id:"mesh",kind:"extrude",profile:[[-1,-1],[1,-1],[0,1]],depth:1}]. Preserve editable recipe revisions and stable node/entity IDs on edits. Follow new-only rules. Do not pair this backend with Blender, and do not claim support for lathe, arbitrary mesh, scripts, URLs, or external assets.`;

...
Execution flow: user prompt → LLM-authored validated modeling program → paired local companion queue → isolated headless Blender job → validated local asset cache → browser GPU rendering. The LLM may run through the selected remote provider, but model construction and scene rendering in this flow use the user’s computer. Stream lightweight reservations immediately while Blender builds the finished asset in the background. Account or provider credentials must never enter the Blender job environment.
...
5. Run the complete real-provider flow on the packaged runtime, including a request that explicitly forbids catalog reuse. Astra reviews worker changes and the acceptance evidence before marking any gate complete; assign independent executor, packaging and integration tasks to Luna sequentially, with at most one worker active at a time.

...
- [x] Baked assets survive export and independent signed-out publication without authoring services or provider credentials. Evidence: live production publication at `docs/evidence/publication-live/` (real account, cloud save, per-Orb Vercel provisioning, deployment serving, signed-out zero-cookie browser playback with data-ready and canvas), plus ZIP-export integrity tests and published-fixture gameplay evidence. Model-backed world creation is separately covered by live provider runs (`docs/evidence/provider-e2e/flagship-openrouter/`, `garden-openrouter/`, `browser-procedural-live-openrouter/`, `browser-deformation-openrouter/`, `browser-vary-openrouter/`).
- [x] Selected kernels/worker assets are pinned, license-reviewed, locally hosted where appropriate, and retained with required notices. Evidence: exact version pins matching `BROWSER_MANIFOLD_KERNEL_VERSION` and a committed lockfile (`cfbe108`), locally hosted kernel/interpreter wasm with license texts and `manifold-provenance.json` in `public/modeling/`, and the deployed worker bytes matching the local build at `docs/evidence/publication-live/`.

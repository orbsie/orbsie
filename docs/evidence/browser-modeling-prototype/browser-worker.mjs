...
const assert=(ok,message)=>{if(!ok) throw Error(message||'Assertion failed')}; assert.equal=(a,b)=>assert(a===b);
const start=performance.now();
const wasm=await Module(); wasm.setup(); initializationComplete(wasm,undefined,true);
self.onmessage=()=>{ const ready=performance.now(); let uid=0;
...
try {
 ...
 self.postMessage(JSON.stringify({scope:'Browser Worker geometry and repeated jobs; no app integration',versions:{bitbybit:'1.1.1',manifold:'3.3.2'},initializationMs:ready-start,constructionAndInspectionMs:performance.now()-ready,arch:a,edited:b,inferenceCalls:0},null,2));

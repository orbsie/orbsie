...
Binary acquisition completed successfully: all 374 archives (290,549,088 bytes) were independently rehashed against the reviewed lock. `binaries.json` retains the completion report and verification scope. Matching source acquisition and runtime assembly remain separate gates. No package was installed on the host.

Offline transport regressions: `PYTHONDONTWRITEBYTECODE=1 /usr/bin/python3 tests/fetch-ubuntu-runtime-inputs.test.py` passed all five tests. They exercise valid publication, wrong-hash/short/oversized responses, invalid cached archives before network access, preservation of another writer's partial file, and redirect rejection. These tests mock signed resolution and transport; authenticated lock integration is covered separately by the dependency-lock suite.

Matching source acquisition is complete: 850 archives for 267 source-package identities, all independently rehashed against the authenticated acquisition plan. `sources.json` records the result. Sources remain in the local optional-runtime cache; this is not evidence that a distributable Blender bundle includes them or that native runtime execution passed. Browser-first modeling remains the primary implementation path.

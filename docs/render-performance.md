...
Normal-motion input follow-up: `docs/evidence/input-feedback-normal-motion/` records DOM feedback at 43.6 ms and the next animation-frame observation at 106.9 ms. The latter misses 100 ms in this sample and remains false in the report. This closes the reduced-motion-only observation gap, not the representative-device performance gate. Functional fixture generation passed with one intercepted request and no browser errors.

## Post-deformation regression observation — 2026-09-09

Source `b445f24` (development server, no rebuild of the deployed bundle) ran the same 14-entity fixture at 1440×1000 with 240 frame intervals and no video recording. The host is unchanged (Linux 6.8.0-139-generic, AMD EPYC 9124, 32 logical CPUs, ~64 GiB RAM) and Chromium 153 still reports SwiftShader. Median 33.4 ms and p95 50.1 ms match the prior no-recording observation, so the twist/taper/vary modeling work did not move shared fixture-frame scheduling. Evidence: `docs/evidence/render-performance-20260909/`. This remains a SwiftShader observation, not native-GPU or sustained-fps acceptance.

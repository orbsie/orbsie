...
Release: app commit `8fc9dd1` deployed successfully to https://orbsie-jnj20jje5-grappeggias-projects.vercel.app and https://orbsie.com after the production migration passed. Read-only production smoke in `evidence/finished-geometry-release/` verified the player/worker hashes, landing page, unauthenticated journal boundary, and no browser errors. Authenticated geometry-recovery proof above used the development database.

Local interruption follow-up (`361f9da`): the browser store now advances its finished-geometry fallback after every accepted operation. Stop and premature stream closure preserve the latest refined shape if a subsequent coarse replacement is unfinished. Two regressions reproduced the old rollback to the pre-request shape, then passed along with the full 447-test suite (7 skipped) and production build. Undo retains the original pre-request history snapshot; durable acknowledgement and active-run guards still precede local application.

The local follow-up is deployed at https://orbsie-ku7rtqisb-grappeggias-projects.vercel.app and https://orbsie.com. Read-only production smoke passed, including exact player/worker hashes, landing rendering and the unauthenticated journal boundary; evidence is in `evidence/local-finished-geometry-release/`.

...
Status: the interpreter foundation is implemented and reviewed in `3ad7a4a`, with ten focused tests and actual browser evidence in `docs/evidence/browser-procedural-foundation-worker/`. Editor/provider integration is in progress; the procedural feature is not deployed or accepted end to end. The interpreter emits existing `BrowserModelRecipe` JSON, including bounded custom mesh nodes. It never receives scene handles, credentials or geometry-kernel objects. Integration must retain versioned source, its seed and a host-computed source hash alongside the validated derived recipe. Successful geometry baking must precede committing a replacement; failure must leave the previous source, recipe and mesh intact.

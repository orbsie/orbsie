...
Composition milestone (local, pending deployment): strict `compose` nodes join 2–32 separated inputs; `mirror` reflects an input through the origin plane defined by a nonzero normal; `linear-array` makes 2–32 copies at `i * offset`; `instances` accepts 1–32 explicit positive-scale XYZ transforms. Output is one baked mesh. These operations do not implement scene parenting, independent child entity IDs/materials, or GPU instancing.

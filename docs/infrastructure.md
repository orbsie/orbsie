...
Production/development authentication secrets, base URLs and storage variables are configured in Vercel. Production also has the team ID and five-Orb publishing quota. Creating a dedicated publication token through the API returned HTTP 403, “Cannot create tokens for this app.” The owner subsequently supplied a dedicated token. Its access to the Orbsie Vercel project was verified with HTTP 200, and it was stored as sensitive production `VERCEL_DEPLOY_TOKEN`. No personal CLI token was copied into the application. Dedicated publish creation still requires database setup before end-to-end verification.

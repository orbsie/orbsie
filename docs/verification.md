...
Latest application production verification: source `edbef92` is deployed at https://orbsie.com (`orbsie-emrv0pyhw-grappeggias-projects.vercel.app`). Public root returned 200, anonymous generation-run access returned 401, and the deployed player runtime SHA-256 exactly matched the local artifact. The public browser loaded the canvas and exact composer placeholder with no page errors or non-GET requests; its screenshot was visually inspected. See [release evidence](evidence/game-runtime-release/report.json).

The current combined suite passed 395 tests, TypeScript and the production build. Six tests were skipped: three opt-in database checks and three Blender runtime checks whose temporary packaged artifacts are absent in this environment. Rendered variable gates, click/color/visibility, position/path actions and restart pass in both editor and ZIP; rule edits during play now show a restart notice that survives later modeling progress. See [game action evidence](evidence/game-actions/README.md) and [rule edit evidence](evidence/game-rule-edits/README.md). A fresh local production-browser flow made two actual ChatGPT `gpt-6-astra` calls with low reasoning/default processing and passed creation, visible scoped recoloring, local reload, ZIP export and input-game scoring/win/loss/restart. See [live ChatGPT evidence](evidence/provider-e2e/chatgpt-authored-input-game/README.md). Production smoke itself made no inference calls. This does not complete the full plan: additional live API-key scenarios, packaged Blender delivery, broader gameplay/performance evidence and dedicated Orb publishing remain open.

## Passing checks

- 395 automated tests and TypeScript checking pass; six environment-dependent/opt-in tests are skipped as detailed above. Coverage includes operation ordering, provider framing, Astra selection/cancellation, immutable archives, gameplay objectives/platform edits, atomic draft preservation, monotonic undo/redo, cloud conflict recovery, publication compare-and-swap, and project-scoped response handling. No model calls occur in this suite.
...
An earlier owner clarification limited Fast to Codex development agents; the current project configuration disables Fast for those agents as well. Orbsie's local model harness explicitly requests standard processing on every thread and turn, retaining Astra low. A deterministic regression verifies that separation; the historical Fast-requested retry has not been rerun.
...
Publication recovery and readiness are implemented beyond the earlier audit notes: deployment recovery now pages Vercel by cursor with a 5×100 bound, validates every immutable metadata field, and fails closed on aborts, network errors, and non-advancing cursors (`tests/publication-recovery.test.ts`). READY promotion fetches the publication manifest, every listed artifact including referenced catalog GLBs, licenses and provenance, validates the project snapshot against the orb world and revision, and promotes only after all checks pass; corrupt artifacts preserve the previous public release and compare-and-swap races are rejected (`tests/publication-regressions.test.ts`). Dedicated publication itself still awaits the corrected Vercel token permissions; no stronger release claim is made. Resend terms/sender configuration remains pending, and no email was sent during this pass.
...
New Orb deployments include a SHA-256 manifest for the exact HTML, player JS/CSS and project snapshot, with its digest stored in Vercel metadata. READY promotion now anonymously fetches and verifies all files, exact project/revision and hashes before the existing compare-and-swap. Protected, redirected, missing or corrupt artifacts preserve the previous release. Legacy deployments without integrity metadata can be republished into a verifiable release. Artifact response sizes/concurrency are bounded; public fetches receive no editor credentials.

Astra reviewed the change and requested the legacy-republish and oversized-artifact fixes before acceptance. The final integrated suite passed 114 tests, with two optional database tests skipped. This is deterministic verification; actual independent Vercel publication remains unverified until the production token's project-creation permission is resolved.

Release `b16509b` is deployed at https://orbsie.com (https://orbsie-8u71mc5aj-grappeggias-projects.vercel.app). Production homepage returned 200, the exact composer and provider settings were visible, and no demo controls remained. The production inspection blocked all non-GET API requests and made no generation calls. Evidence: `evidence/real-generation-release/production.json` and `connections.png`.
